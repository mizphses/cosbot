...
projects.post('/:id/new', async (c) => {
  ...
  try {
    c.env.COS_TESTCASES.put(caseId, content)
  } catch (e) {
    console.error(e)
  }
...
projects.get('/cases/:caseId/view', async (c) => {
  const id = c.req.param('caseId')
  const html = (await c.env.COS_TESTCASES.get(id)) || ''
